...
We evaluate PIMSM across temporal-context truncation, low-resource transfer, brain-state shift, and cross-domain forecasting (Weather-5K spatial OOD with held-out stations), showing on completed HCP experiments that physics-informed multi-scale temporal structure improves robustness and representation stability over parameter-matched single-scale baselines without shift-specific adaptation.
\end{itemize}
...
We evaluate across four axes: \textbf{Axis (i)} temporal-context shift, implemented as early-window truncation of task trials while retaining full-sequence spectral calibration; \textbf{Axis (ii)} low-resource transfer; \textbf{Axis (iii)} brain-state shift (resting $\leftrightarrow$ task); and \textbf{Axis (iv)} cross-domain shift (Weather-5K spatial OOD forecasting on held-out stations).
...
\textbf{Weather-5K}~\citep{han2024far}: 5,000 global weather stations; cross-domain benchmark using hourly multivariate forecasting under a spatial OOD split, where geographically isolated held-out stations are never observed during training. Calendar-year splits are used only as a soft temporal-OOD reference because the train/test years are disjoint rather than a strict in-distribution control.
...
All models are trained under a shared protocol with matched parameter budgets across baselines. Full optimization settings, Weather-5K preprocessing, RevIN normalization, and OOD split construction are reported in Appendix~\ref{sec:appx:training_details} and Appendix~\ref{sec:appx:data}; architecture dimensions are reported in Appendix~\ref{sec:appx:arch_details}.


\section{Results}\label{sec:results}

We evaluate PIMSM across four axes of structured distribution shift. Axes (i--ii) use HCP motor fMRI; axis (iii) uses HCP resting-state$\to$task fMRI; axis (iv) uses Weather-5K. All comparisons use parameter-matched baselines (Mamba2, Transformer, 1D-CNN; $\approx$7.85M parameters each).
...
Table~\ref{tab:weather} reports variable-wise MAE on temperature, sea-level pressure, dew point, wind rate, and wind angle components under the spatial OOD split, where test stations are held out from training.
...
We do not include the calendar split in the main table: although it is commonly reported as an in-distribution split, the train/test calendar separation makes it closer to a soft temporal-OOD setting than a strict ID control, whereas the spatial split more directly tests cross-domain station generalization.
...
Variable-wise OOD spatial MAE is reported for temperature (TMP), sea-level pressure (SLP), dew point (DEW), wind rate, and wind angle components (cos/sin) across 24h, 72h, 120h, and 168h forecasting horizons. PIMSM is applied without domain-specific adaptation.
}
\label{tab:weather}
\end{table}
...
The current experiments test temporal-context truncation rather than a scanner-level change in TR; SpectralHyperNet receives the full sequence for scale calibration while the SSM backbone receives the shortened early-window input. This setting is useful for testing whether full-sequence spectral structure stabilizes representations under limited task evidence, but it should not be interpreted as a complete evaluation of acquisition-rate shift. PIMSM also assumes that a small number of spectrum-derived knees provides a useful summary of the signal's temporal organization; domains without clear piecewise spectral structure may require different scale parameterizations. The present model fixes $K=3$ scale regimes in the main experiments, so broader sensitivity over the number of knees remains future work. The acquisition-unit anchoring also requires a known sampling interval or physically meaningful time unit. SpectralHyperNet adds FFT and spectral-fit overhead relative to plain Mamba2, and the current manuscript does not provide a full GPU-hour accounting. Finally, the HCP tables use three seeds and report standard deviations rather than formal paired significance tests; the related multi-scale SSM/Mamba methods are discussed as contemporaneous related work but are not included as parameter-matched HCP baselines in the current tables. The Weather-5K evaluation focuses on spatial OOD generalization across held-out stations; calendar-year splits are treated as a soft temporal-OOD reference rather than as the primary in-distribution control.
...
On completed HCP experiments, PIMSM improves robustness over parameter-matched single-scale baselines under severe temporal-context truncation, extreme low-resource transfer, and resting-state$\to$task-state generalization. Weather-5K spatial-OOD results suggest that the same spectrum-guided temporal parameterization can extend to meteorological forecasting across held-out stations. Together, these results support the view that robust sequence modeling of multiscale physical signals benefits from architectural alignment with the intrinsic temporal organization of the underlying dynamical system.
