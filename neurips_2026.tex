...
\begin{abstract}
Scientific foundation models are expected to reuse representations across datasets, acquisition protocols, and deployment domains, but natural dynamical systems---from neural circuits to atmospheric processes---contain interacting processes across multiple timescales. We argue that \emph{temporal-scale alignment} is a practical requirement for foundation models deployed on multiscale scientific time series: a model should not merely fit in-distribution dynamics, but should preserve relevant temporal structure under acquisition, context, and domain shifts. We study one failure mode, \emph{temporal kernel mismatch}, in which the effective memory policy learned by a sequence model is not anchored to the signal's physical timescales, contributing to representation drift and degraded transfer. We propose \textbf{Physics-Informed Multi-Scale Mamba (PIMSM)}, a state-space architecture that maps spectrum-estimated knee frequencies to scale-specific discretization parameters and anchors them to acquisition time units. On Human Connectome Project fMRI, PIMSM improves robustness and representation stability over parameter-matched baselines under severe temporal-context truncation, extreme low-resource transfer, and resting-state to task-state generalization. We further test the same architecture, without modality-specific adaptation, on Weather-5K hourly meteorological forecasting under a spatial OOD split with held-out stations. PIMSM attains the lowest variable-wise MAE across all reported horizons and weather variables, suggesting that explicit temporal-scale alignment is a modality-agnostic inductive bias for multiscale scientific foundation models.
\end{abstract}

\section{Introduction}
\label{sec:introduction}

Scientific foundation models promise reusable representations across datasets, acquisition protocols, and downstream tasks. For multiscale scientific time series, the next bottleneck is not simply larger pretraining: the model must preserve the temporal scales that make the representation reusable under shift. Neural recordings exhibit scale-free or multifractal temporal structure~\citep{he2011scale,bae2025spatiotemporal}, and atmospheric measurements show power-law spectral regimes~\citep{cheng2020power}. This raises a practical modeling question: how should a sequence backbone represent temporal scales that are visible in the signal spectrum and stable across related acquisition contexts?

Scientific deployment also makes distribution shift routine rather than exceptional. Temporal resolution, sampling rate, scanner/site, brain state, season, station, and available labels can change between training and use, while many foundation-model claims are still driven by in-distribution or narrowly shifted evaluations. Recent brain and forecasting foundation models show the value of large-scale representation learning and low-data adaptation~\citep{tak2026brainiac,wang2025towards,wang2025slim,wang2026omni}, while Weather-5K highlights the gap between academic time-series models and operational weather forecasting~\citep{han2024far}. We ask a complementary question: after scale and pretraining, what architectural constraint keeps scientific representations stable under deployment shift?
...
We evaluate PIMSM across four axes of structured distribution shift spanning neural and meteorological domains: temporal-context truncation and low-resource generalization on HCP motor fMRI, brain-state shift (resting-state $\to$ task fMRI), and cross-domain transfer to Weather-5K spatial OOD forecasting without adaptation. These axes map to practical deployment stresses: low-latency neural decoding, scarce task labels, abundant resting-state but limited task-fMRI data, and long-horizon forecasting at geographically held-out stations. The Weather-5K axis deliberately changes both modality and deployment geography, testing whether a spectrum-to-dynamics prior learned as an architectural principle rather than a domain-specific module remains useful outside neuroimaging. On HCP, PIMSM improves robustness and representation stability over parameter-matched single-scale baselines; on Weather-5K, it gives consistent variable-wise gains across forecasting horizons.
...
We evaluate four axes: \textbf{Axis (i)} temporal-context truncation of task trials with full-sequence spectral calibration; \textbf{Axis (ii)} low-resource transfer; \textbf{Axis (iii)} brain-state shift (resting $\leftrightarrow$ task); and \textbf{Axis (iv)} Weather-5K spatial OOD forecasting on held-out stations.
...
A single effective timescale implements a limited memory policy: fast updates track short-lived changes but amplify high-frequency fluctuations, while slow updates preserve context but can miss rapid task-relevant changes.
When TR, context length, brain state, or station regime changes, the useful balance between slow and fast structure can shift, reconfiguring the latent geometry.
PIMSM addresses this by maintaining multiple spectrum-derived memory policies rather than forcing one dominant kernel to serve all regimes.

\subsection{Kernel mismatch intuition}
\label{sec:theory:kernelbound}

Many SSM-based encoders admit a locally linear convolutional view:
\begin{equation}
...
\textbf{Weather-5K}~\citep{han2024far}: 5,000 global weather stations; cross-domain benchmark using hourly multivariate forecasting under a spatial OOD split, where geographically isolated held-out stations are never observed during training. We prioritize this split because Weather-5K is motivated by real-world station forecasting, and held-out stations more directly test deployment to unseen geographic domains than chronological holdouts over already observed stations. This setting is deliberately modality-agnostic: the same temporal-scale parameterization used for fMRI is applied to meteorological variables sampled every hour.
...
\textbf{Spectral modeling.} Given the full sequence, we estimate the power spectral density $P(f)$ via FFT. In HCP motor experiments, SpectralHyperNet receives the full trial for spectral calibration while the SSM backbone receives only the first 3 or 2 TRs, testing whether full-sequence spectral structure stabilizes shortened task evidence rather than simulating a changed scanner TR. We assume a multi-scale power-law structure:
\begin{equation}
...
\textbf{Knee estimation.} SpectralHyperNet predicts $(f_1, f_2)$ and exponents $(\beta_1, \beta_2, \beta_3)$ from the full-sequence spectrum, initialized from a data-driven scipy fit on the first training batch. A multi-resolution MAE spectral fit loss drives knee placement and a seam loss encourages piecewise continuity; hyperparameters are in Appendix~\ref{sec:appx:training_details}.

\textbf{Energy-weighted center frequency and discretization mapping.} For each band $k$, we compute an energy-weighted center frequency $f_{c,k}$ under $P(f) \propto f^{-\beta}$, giving a representative frequency that reflects spectral mass rather than a simple geometric mean.
...
\textbf{Acquisition-unit anchoring.} To reduce $\Delta$--$A$ scale non-identifiability, $\Delta_k$ is expressed in dataset-native time units with log-scale regularization, while $A$ is initialized and regularized to prevent compensatory rescaling. This pins $\tau_\text{eff} \approx \Delta/(-A)$ from both sides; implementation details are in Appendix~\ref{sec:appx:arch_details}.
...
This setting is directly relevant to low-latency brain-computer interfaces and closed-loop neurotechnology, where decisions must often be made from the earliest available neural evidence rather than from full trials.
...
Table~\ref{tab:drift2tr} shows that PIMSM maintains the highest accuracy ($0.933\pm0.016$) and representation stability (CKA $0.812$, dCor $0.846$) under severe truncation, compared with Mamba2 (accuracy $0.914$, CKA $0.733$), Transformer ($0.884$, CKA $0.783$), and 1D-CNN ($0.847$, CKA $0.559$). Replacing spectrum-derived $\Delta$ with learnable or random values increases the full-trial$\to$2TR accuracy drop (0.069 and 0.094 versus 0.054 for PIMSM) and lowers representation stability, indicating that robustness is not explained by multi-scale capacity alone but by spectrum-aligned temporal anchoring.
...
This setting reflects a common neuroimaging deployment constraint: task-fMRI labels are expensive and often scarce, so useful models must adapt from very small labeled cohorts.
...
This matches a practical asymmetry in neuroimaging: resting-state scans are broadly available, whereas task-fMRI labels and paradigms are often more limited and heterogeneous.
...
We focus on spatial OOD because a chronological split still evaluates future windows from stations seen during training, whereas held-out stations test whether temporal-scale alignment transfers to unseen geographic regimes.
...
The widening margin at longer horizons is consistent with the intended real-world setting: robust forecasting under station shift increasingly depends on preserving longer-range temporal structure rather than fitting short-horizon local patterns alone.
...
Recent work in brain foundation models and time-series adaptation emphasizes generalizability across data-efficiency, temporal/acquisition shift, and cross-domain settings~\citep{tak2026brainiac,qin2025cora,han2026unica,wang2025towards,wang2025slim,wang2026omni}.
PIMSM asks a different question from simply scaling pretraining: whether a sequence backbone preserves the temporal scales needed for reuse under shift.
Rather than using frozen backbones, auxiliary adaptation modules, or shift-specific supervision, it targets robustness through a temporal-scale inductive bias.

\paragraph{Multi-scale state space models.}
Ms-Mamba~\citep{karadag2025ms}, MS-SSM~\citep{Karami2025MSSSMAM}, STM3~\citep{chen2025stm3}, and WMF-Traffic~\citep{li2025multi} show that multi-scale SSM/Mamba designs are useful for forecasting and generic sequence benchmarks.
PIMSM differs by deriving scale-specific discretization from empirical spectra, enforcing an ordered temporal hierarchy, and anchoring $\Delta$ and $A$ in dataset-native acquisition units.

\section{Limitations}
\label{sec:limitations}

The current experiments test temporal-context truncation rather than a scanner-level change in TR: SpectralHyperNet receives the full sequence for scale calibration while the SSM backbone receives shortened early-window input. PIMSM also assumes that a small number of spectrum-derived knees usefully summarizes temporal organization; domains without clear piecewise spectral structure, or tasks requiring a different number of scales than the fixed $K=3$ used here, may require different parameterizations. Acquisition-unit anchoring requires a known sampling interval, and SpectralHyperNet adds FFT and spectral-fit overhead relative to plain Mamba2; full GPU-hour accounting remains future work. Finally, HCP tables report three-seed means and standard deviations rather than formal paired significance tests, contemporaneous multi-scale SSM/Mamba methods are not included as parameter-matched HCP baselines, and Weather-5K is evaluated under held-out-station spatial OOD.
