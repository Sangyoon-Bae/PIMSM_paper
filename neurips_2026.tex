...
PIMSM achieves the highest accuracy and $3$--$5\times$ lower cross-seed variance than the baselines in this setting, consistent with the hypothesis that physics-informed multi-scale kernels reduce sensitivity to resting$\to$task reconfiguration.
}
...
The 72h result provides an initial cross-domain check: PIMSM achieves the lowest MAE on all six variables, with the largest gains on scalar meteorological variables (TMP, SLP, DEW) and smaller but consistent gains on wind components.
Because the 24h, 120h, and 168h PIMSM runs are still in progress, we treat the Weather-5K evidence as partial rather than a completed benchmark.
...
Variable-wise OOD spatial MAE is reported for temperature (TMP), sea-level pressure (SLP), dew point (DEW), wind rate, and wind angle components (cos/sin) across 24h, 72h, 120h, and 168h forecasting horizons. PIMSM is applied without domain-specific adaptation; pending PIMSM horizons are left blank.
}
\label{tab:weather}
\end{table}
...
On completed HCP experiments, PIMSM improves robustness over parameter-matched single-scale baselines under severe temporal-context truncation, extreme low-resource transfer, and resting-state$\to$task-state generalization. Initial Weather-5K spatial-OOD results at the 72h horizon suggest that the same spectrum-guided temporal parameterization can extend to meteorological forecasting, with the remaining horizon sweep still in progress. Together, these results support the view that robust sequence modeling of multiscale physical signals benefits from architectural alignment with the intrinsic temporal organization of the underlying dynamical system.
