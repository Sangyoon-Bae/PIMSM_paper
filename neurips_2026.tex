...
\textbf{Spectral modeling.} Given a spectral context sequence $x^{\mathrm{spec}}$, we estimate the power spectral density $P(f)$ via FFT and fit Eq.~\eqref{eq:piecewise_powerlaw}. By default, $x^{\mathrm{spec}}$ is the same sequence processed by the SSM backbone; in HCP early-window experiments, SpectralHyperNet receives the full trial while the SSM backbone receives only the first 3 or 2 TRs. This tests whether full-sequence spectral structure stabilizes shortened task evidence, not a changed scanner TR. In the default $K=3$ setting, $(f_1, f_2)$ denote the two knees separating three regimes.
...
Sex classification is used because the label is scan-condition independent and available for both resting-state and task-fMRI, allowing the evaluation to change brain/acquisition state while keeping the target fixed.
...
These limitations leave a clear deployment target. PIMSM addresses several recurring real-world constraints with one mechanism: short recordings in low-latency BCI, small labeled cohorts in task-fMRI, difficult resting$\leftrightarrow$task reuse despite abundant resting-state data, and spatial OOD forecasting for remote or sparsely observed weather stations. This suggests that physics-informed temporal parameterization can serve as a reusable design principle for models that must operate under deployment shifts, not only curated in-distribution benchmarks.
