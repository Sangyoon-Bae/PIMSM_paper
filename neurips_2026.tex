...
The implementation first restricts subjects to those with both resting-state and HCPMMP1 motor-task files, forms a random subject split, and uses resting-state runs for train/validation while replacing the held-out test modality with motor-task fMRI.
Sex classification is used because the Gender metadata label is scan-condition independent and available for both resting-state and task-fMRI, allowing the evaluation to change brain/acquisition state while keeping the target fixed.
...
These limitations leave a clear deployment target. PIMSM addresses several recurring real-world constraints with one mechanism: short recordings in low-latency BCI, small labeled cohorts in task-fMRI, difficult resting$\to$task reuse despite abundant resting-state data, and spatial OOD forecasting for remote or sparsely observed weather stations. This suggests that physics-informed temporal parameterization can serve as a reusable design principle for models that must operate under deployment shifts, not only curated in-distribution benchmarks.
