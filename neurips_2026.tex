...
\begin{abstract}
Natural dynamical systems---from neural circuits to atmospheric processes---often contain interacting processes across multiple timescales and exhibit piecewise power-law spectral structure, spanning fast local fluctuations to slow global regulation. Standard sequence backbones can model such signals, but their temporal kernels are typically learned without an explicit link to the physical timescales present in the data. We study this gap as \emph{temporal kernel mismatch}: under changes in acquisition, context, or domain, the effective memory policy learned by a model may no longer align with the signal's relevant temporal scales, contributing to representation drift and degraded transfer. We propose \textbf{Physics-Informed Multi-Scale Mamba (PIMSM)}, a state-space architecture that maps spectrum-estimated knee frequencies to scale-specific discretization parameters and anchors them to acquisition time units. This yields a compact hierarchy of temporal kernels grounded in empirical spectral structure. On Human Connectome Project fMRI, PIMSM improves robustness and representation stability over parameter-matched baselines under severe temporal-context truncation, extreme low-resource transfer, and resting-state to task-state generalization. We further test the same architecture, without modality-specific adaptation, on Weather-5K hourly meteorological forecasting under a geo-spatial OOD split with held-out stations. PIMSM attains the lowest variable-wise MAE across all reported horizons and weather variables, suggesting that explicit temporal-scale alignment is a modality-agnostic inductive bias for multiscale scientific time series.
\end{abstract}

\section{Introduction}
\label{sec:introduction}

Scientific foundation models promise reusable representations across datasets, acquisition protocols, and downstream tasks. Yet scientific time series are generated by physical systems with coupled dynamics across scales: neural recordings exhibit scale-free or multifractal temporal structure~\citep{he2011scale,bae2025spatiotemporal}, and atmospheric measurements show power-law spectral regimes~\citep{cheng2020power}. This raises a practical modeling question: how should a sequence backbone represent temporal scales that are visible in the signal spectrum and stable across related acquisition contexts?
...
We frame one failure mode as \textbf{temporal kernel mismatch}. A model can fit the average temporal statistics of an in-distribution split while learning an effective memory policy that is poorly anchored to the physical timescales of the data. When the temporal context or acquisition condition changes, this policy may become less aligned with task-relevant dynamics, producing representation drift and degraded transfer. This framing motivates architectures that expose and constrain temporal scale, rather than leaving all scale selection to unconstrained end-to-end training.
...
This issue is concrete in state-space models (SSMs). In Mamba-style selective SSMs, $\Delta$ affects memory and update speed~\citep{gu2023mamba}, but it is not a physical sampling interval by itself: the induced kernel depends jointly on $\Delta$ and $A$ (e.g., through $\exp(\Delta A)$), so the two can compensate for each other. Without additional structure, an SSM can fit an in-distribution kernel while providing little control over how its effective timescales relate to acquisition units or spectral regimes.

Based on this observation, we introduce \textbf{Physics-Informed Multi-Scale Mamba (PIMSM)}. PIMSM replaces a fully unconstrained temporal parameterization with a spectrum-guided hierarchy of kernels whose scales are anchored to acquisition time. Rather than treating multi-scale structure as an external preprocessing step, PIMSM uses the observed spectral hierarchy to parameterize the state-space dynamics.

We evaluate PIMSM across four axes of structured distribution shift spanning neural and meteorological domains: temporal-context truncation and low-resource generalization on HCP motor fMRI, brain-state shift (resting-state $\to$ task fMRI), and cross-domain transfer to Weather-5K geo-spatial OOD forecasting without adaptation. This last axis deliberately changes both modality and deployment geography: models are trained on one set of weather stations and evaluated on geographically held-out stations, testing whether a spectrum-to-dynamics prior learned as an architectural principle rather than a domain-specific module remains useful outside neuroimaging. On HCP, PIMSM improves robustness and representation stability over parameter-matched single-scale baselines; on Weather-5K, it gives consistent variable-wise gains across forecasting horizons.

Our main contributions are as follows:

\begin{itemize}
\item \textbf{Temporal kernel mismatch as a diagnostic for shift sensitivity.}
We formulate temporal kernel mismatch as a useful lens for analyzing representation drift in scientific time-series models, connecting changes in temporal context or acquisition condition to changes in the effective memory policy required by the signal.
...
We evaluate PIMSM across temporal-context truncation, low-resource transfer, brain-state shift, and cross-domain forecasting (Weather-5K geo-spatial OOD with held-out stations), showing that physics-informed multi-scale temporal structure improves robustness across both neural and meteorological time series without shift-specific adaptation.
\end{itemize}
...
We evaluate across four axes: \textbf{Axis (i)} temporal-context shift, implemented as early-window truncation of task trials while retaining full-sequence spectral calibration; \textbf{Axis (ii)} low-resource transfer; \textbf{Axis (iii)} brain-state shift (resting $\leftrightarrow$ task); and \textbf{Axis (iv)} cross-domain shift (Weather-5K geo-spatial OOD forecasting on held-out stations).
...
\subsection{Why a single effective timescale can be fragile under shift}
\label{sec:theory:intuition}

\paragraph{Core intuition.}
A sequence model whose effective dynamics concentrate around one dominant timescale implements a limited memory policy.
...
A broad class of locally linear temporal encoders admits a kernel representation (Section~\ref{sec:theory:kernelbound}); for intuition, suppose the dominant component of the effective kernel is a single exponential with rate $\lambda>0$:
\begin{equation}
...
Under acquisition or context shifts (e.g., changed TR, sampling rate, preprocessing, or available temporal window), the relative contribution and spectral profile of $(x_s, x_f)$ can change, so the most useful memory policy may also change, leading to a reconfigured latent geometry.
...
This structure allows the model to preserve slow components while remaining responsive to faster components, reducing sensitivity to shifts that rebalance $x_s$ and $x_f$.
In our setting, the scales are not engineered by ad-hoc filtering; instead, they are parameterized from the observed spectrum via knee frequencies and exponents (Section~\ref{sec:methods}), yielding a data-grounded multi-scale dynamics that aims to stabilize latent geometry under shift.
...
We use this view to motivate why mismatch in temporal kernels can translate into representation error.
...
Lemma~\ref{lem:kernel_mismatch} implies that poor approximation of the condition-specific kernel, i.e., large $\|g_{e'}-\tilde g\|_1$, upper-bounds how much representation error can be controlled through the temporal encoder alone.
Conversely, enlarging the kernel family in a structured manner can reduce achievable mismatch and provides one route to lower representation drift.
...
\end{equation}
showing how temporal-kernel mismatch can propagate to prediction instability under shift.
...
A model whose effective dynamics concentrate around one scale corresponds to a relatively narrow kernel family (e.g., one dominant exponential mode or one ``corner'' in frequency space), which may not uniformly approximate kernels consistent with piecewise scaling across acquisition conditions.
A mixture of exponentials provides a richer, yet still structured, family:
\begin{equation}
...
The target is not the stability of the raw parameters per se, but the stability of the induced latent geometry under shift.
...
While the kernel mismatch bound (Eq.~\eqref{eq:kernel_mismatch_bound}) is stated for acquisition-preserving shifts, the same intuition is relevant to brain-state shifts (Axis iii): a transition from resting to task state alters the spectral composition of BOLD signals and may change the useful effective kernel. A model trained on resting-state dynamics can therefore require reconfiguration when deployed on task fMRI. A physics-informed multi-scale kernel family, by covering a broader spectral range, is structurally better positioned to remain stable across such generative regime changes.
...
\textbf{Weather-5K}~\citep{han2024far}: 5,000 global weather stations; cross-domain benchmark using hourly multivariate forecasting under a geo-spatial OOD split, where geographically isolated held-out stations are never observed during training. This setting is deliberately modality-agnostic: the same temporal-scale parameterization used for fMRI is applied to meteorological variables sampled every hour. Calendar-year splits are used only as a soft temporal-OOD reference because the train/test years are disjoint rather than a strict in-distribution control.
...
\textbf{Spectral modeling.} Neural signals are known to exhibit piecewise power-law spectra. Given the full sequence, we estimate the power spectral density $P(f)$ via FFT. In HCP motor experiments, this means SpectralHyperNet receives the full trial sequence for spectral calibration, while the temporal-context truncation experiment restricts the SSM backbone input to the first 3 or 2 TRs. Thus Axis (i) tests whether a full-sequence estimate of the signal's spectral hierarchy can stabilize representations when the task evidence available to the sequence backbone is severely shortened; it is not intended to simulate a changed scanner TR. We assume a multi-scale power-law structure:
\begin{equation}
...
\textbf{Knee estimation.} A lightweight neural network (SpectralHyperNet) predicts $(f_1, f_2)$ and scaling exponents $(\beta_1, \beta_2, \beta_3)$ directly from the full-sequence power spectrum via FFT.
...
All models are trained under a shared protocol with matched parameter budgets across baselines. Full optimization settings, Weather-5K preprocessing, RevIN normalization~\citep{kim2021reversible}, and OOD split construction are reported in Appendix~\ref{sec:appx:training_details} and Appendix~\ref{sec:appx:data}; architecture dimensions are reported in Appendix~\ref{sec:appx:arch_details}.


\section{Results}\label{sec:results}

We evaluate PIMSM across four axes of structured distribution shift. Axes (i--ii) use HCP motor fMRI; axis (iii) uses HCP resting-state$\to$task fMRI; axis (iv) uses Weather-5K geo-spatial OOD forecasting. All comparisons use parameter-matched baselines (Mamba2, Transformer, 1D-CNN; $\approx$7.85M parameters each).
...
Table~\ref{tab:drift2tr} shows that PIMSM maintains the highest accuracy ($0.933\pm0.016$) and representation stability (CKA $0.812$, dCor $0.846$) under severe truncation, compared with Mamba2 (accuracy $0.914$, CKA $0.733$), Transformer ($0.884$, CKA $0.783$), and 1D-CNN ($0.847$, CKA $0.559$). PIMSM ablations with learnable or random $\Delta$ are less stable, suggesting that the spectrum-derived scale parameterization contributes beyond multi-scale capacity alone.
Across the three paired seeds, PIMSM improves over Mamba2 on all shifted 2TR metrics (accuracy, CKA, and dCor).
Given the small seed count, we treat paired tests as exploratory rather than definitive; the paired effect sizes are nevertheless large in this temporal-context setting.
...
PIMSM achieves the highest accuracy and representation stability. Ablations with learnable or random $\Delta$ indicate that spectrum-derived scale parameters are important for the observed robustness.
}
\label{tab:drift2tr}
\end{table}
...
At 1\% data, PIMSM achieves $0.797\pm0.040$, compared with Mamba2 ($0.752$), Transformer ($0.774$), and 1D-CNN ($0.709$).
Differences diminish beyond 10\%, and Transformer is strongest at 5\%, indicating that physics-informed multi-scale dynamics primarily improve sample efficiency in the most extreme low-supervision regime rather than uniformly dominating all label budgets.
...
Models are trained for sex classification on HCP resting-state fMRI and evaluated on HCP motor-task fMRI without adaptation.
...
We evaluate whether PIMSM's physics-informed multi-scale temporal structure---grounded in empirical spectral scaling across domains---transfers to meteorological dynamics without any domain-specific adaptation.
...
Table~\ref{tab:weather} reports variable-wise MAE on temperature, sea-level pressure, dew point, wind rate, and wind angle components under the geo-spatial OOD split, where test stations are held out from training.
This is a substantially different modality and sampling regime from fMRI (hourly station measurements rather than sub-second BOLD-derived ROI sequences), yet the same spectrum-guided temporal parameterization is used.
...
This result supports the interpretation that PIMSM is not simply an fMRI-specific architecture, but a modality-agnostic mechanism for aligning SSM memory scales to empirical temporal structure.
We do not include the calendar split in the main table: although it is commonly reported as an in-distribution split, the train/test calendar separation makes it closer to a soft temporal-OOD setting than a strict ID control, whereas the geo-spatial split more directly tests cross-domain station generalization.
...
\toprule
Horizon & Model & TMP & SLP & DEW & Wind Rate & Wind Angle (cos) & Wind Angle (sin) \\
\midrule
24h & Mamba2      & 1.823 & 2.486 & 1.951 & 1.903 & 0.561 & 0.530 \\
    & 1D-CNN      & 1.150 & 1.505 & 1.316 & 1.330 & 0.416 & 0.389 \\
    & Transformer & 1.351 & 2.066 & 1.417 & 1.537 & 0.462 & 0.450 \\
    & \textbf{PIMSM} & \textbf{0.921} & \textbf{1.190} & \textbf{1.066} & \textbf{1.301} & \textbf{0.410} & \textbf{0.370} \\
\midrule
72h & Mamba2      & 1.827 & 3.984 & 2.513 & 1.949 & 0.587 & 0.591 \\
    & 1D-CNN      & 1.541 & 2.931 & 1.855 & 1.569 & 0.479 & 0.482 \\
    & Transformer & 2.266 & 3.911 & 2.017 & 1.732 & 0.533 & 0.513 \\
    & \textbf{PIMSM} & \textbf{1.167} & \textbf{2.214} & \textbf{1.411} & \textbf{1.512} & \textbf{0.475} & \textbf{0.439} \\
\midrule
120h & Mamba2      & 2.469 & 4.522 & 2.797 & 2.010 & 0.594 & 0.624 \\
     & 1D-CNN      & 1.790 & 3.390 & 2.222 & 1.633 & 0.508 & 0.527 \\
     & Transformer & 2.108 & 4.629 & 2.312 & 1.818 & 0.551 & 0.580 \\
     & \textbf{PIMSM} & \textbf{1.263} & \textbf{2.585} & \textbf{1.546} & \textbf{1.565} & \textbf{0.494} & \textbf{0.457} \\
\midrule
168h & Mamba2      & 2.588 & 4.848 & 2.922 & 2.021 & 0.596 & 0.627 \\
     & 1D-CNN      & 2.009 & 3.748 & 2.516 & 1.667 & 0.523 & 0.556 \\
     & Transformer & 2.159 & 4.724 & 2.294 & 1.747 & 0.537 & 0.580 \\
     & \textbf{PIMSM} & \textbf{1.334} & \textbf{2.849} & \textbf{1.638} & \textbf{1.610} & \textbf{0.506} & \textbf{0.476} \\
\bottomrule
\end{tabular}
\caption{
\textbf{Weather-5K cross-domain forecasting (Axis iv).}
Variable-wise geo-spatial OOD MAE is reported for temperature (TMP), sea-level pressure (SLP), dew point (DEW), wind rate, and wind angle components (cos/sin) across 24h, 72h, 120h, and 168h forecasting horizons. PIMSM is applied without domain-specific adaptation.
}
\label{tab:weather}
\end{table}
...
Unlike adaptation-based approaches, PIMSM targets robustness through architectural inductive bias, without frozen backbones, auxiliary adaptation modules, or shift-specific supervision in the evaluated HCP settings.

\paragraph{Multi-scale state space models.}
Ms-Mamba~\citep{karadag2025ms}, MS-SSM~\citep{Karami2025MSSSMAM}, and STM3~\citep{chen2025stm3} incorporate multi-scale structure into Mamba-based or SSM architectures via parallel scale branches, multi-resolution processing, learnable temporal scales, or mixture-of-experts routing. WMF-Traffic~\citep{li2025multi} uses explicit wavelet decomposition with scale-adaptive convolutions for traffic forecasting.
These methods provide important evidence that multi-scale SSMs are useful, but they generally learn or route scale structure from the task loss and are designed for forecasting or generic sequence benchmarks rather than acquisition-unit-stable neural representation learning.
PIMSM is complementary: it uses empirical spectra to derive scale-specific discretization, enforces an ordered temporal hierarchy, and jointly anchors $\Delta$ and $A$ in dataset-native acquisition units.

\section{Limitations}
\label{sec:limitations}

The current experiments test temporal-context truncation rather than a scanner-level change in TR; SpectralHyperNet receives the full sequence for scale calibration while the SSM backbone receives the shortened early-window input. This setting is useful for testing whether full-sequence spectral structure stabilizes representations under limited task evidence, but it should not be interpreted as a complete evaluation of acquisition-rate shift. PIMSM also assumes that a small number of spectrum-derived knees provides a useful summary of the signal's temporal organization; domains without clear piecewise spectral structure may require different scale parameterizations. The present model fixes $K=3$ scale regimes in the main experiments, so broader sensitivity over the number of knees remains future work. The acquisition-unit anchoring also requires a known sampling interval or physically meaningful time unit. SpectralHyperNet adds FFT and spectral-fit overhead relative to plain Mamba2, and the current manuscript does not provide a full GPU-hour accounting. Finally, the HCP tables use three seeds and report standard deviations rather than formal paired significance tests; the related multi-scale SSM/Mamba methods are discussed as contemporaneous related work but are not included as parameter-matched HCP baselines in the current tables. The Weather-5K evaluation focuses on geo-spatial OOD generalization across held-out stations; calendar-year splits are treated as a soft temporal-OOD reference rather than as the primary in-distribution control.


\section{Conclusion}
\label{sec:conclusion}

We introduced \textbf{Physics-Informed Multi-Scale Mamba (PIMSM)}, motivated by the hypothesis that temporal-kernel alignment is important for generalization in multiscale physical signals. By coupling discretization parameters to spectrum-derived knee frequencies and jointly anchoring both $\Delta$ and $A$ in acquisition time units, PIMSM constructs a structured multi-scale kernel family for temporal-context, brain-state, and cross-domain shifts.

On completed HCP experiments, PIMSM improves robustness over parameter-matched single-scale baselines under severe temporal-context truncation, extreme low-resource transfer, and resting-state$\to$task-state generalization. Weather-5K geo-spatial OOD results show that the same spectrum-guided temporal parameterization extends to meteorological forecasting across held-out stations without modality-specific adaptation. Together, these results support the view that robust sequence modeling of multiscale physical signals benefits from architectural alignment with the intrinsic temporal organization of the underlying dynamical system.
...
\bottomrule
\end{tabular}
\caption{Drift intervention: full trial $\rightarrow$ 2 TR.}
...
\bottomrule
\end{tabular}
\caption{Drift intervention: full trial $\rightarrow$ 3 TR.}
...
For matched experiments we use a 100-station geo-stratified subset. Stations are sampled from geographic clusters built from latitude, longitude, elevation, and station quality metadata, with a target cluster size of eight stations. The geo-spatial OOD split then partitions stations, not windows: stations are clustered on unit-sphere geographic coordinates, clusters are ranked by geographic isolation, and the most isolated cluster(s) are assigned to the held-out test set. In the current runs, 90\% of stations are used for training/validation and 10\% are held out for geo-spatial OOD testing. Within the train-station set, $1/9$ of windows are used for validation, yielding an approximate 8:1:1 train/validation/test station-window budget while keeping test stations disjoint from training.

We also maintain a calendar-year split for reference, with 2014--2020 for training, 2021 for validation, and 2022--2023 for testing. Because this split separates train and test by calendar time, we treat it as a soft temporal-OOD reference rather than a strict in-distribution control. The main Weather-5K table therefore focuses on the geo-spatial OOD split, which more directly tests station-domain generalization.

Normalization statistics for z-scoring are computed from training inputs only. For train stations, per-station mean and standard deviation are estimated from training windows; held-out geo-spatial OOD stations fall back to the global training statistics because their station-specific statistics are unavailable during training. RevIN is then applied on top of this dataset-level normalization during model training and evaluation, and predictions are mapped back to variable units before reporting MAE.
