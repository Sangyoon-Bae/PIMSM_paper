...
\begin{abstract}
Many natural dynamical systems---from neural circuits to atmospheric processes---operate through interacting processes across multiple timescales and exhibit piecewise power-law spectral structure, spanning fast local fluctuations to slow global regulation. Yet dominant sequence models are single-scale, becoming structurally mismatched to a signal's temporal statistics under distribution shift. We argue that time-series generalization is limited by \emph{temporal kernel mismatch}: when acquisition or domain conditions change, single-scale models can incur representation drift and degraded transfer. To address this, we propose \textbf{Physics-Informed Multi-Scale Mamba (PIMSM)}, a state-space architecture that derives scale-specific discretization parameters from spectrum-estimated knee frequencies and anchors them to acquisition time units, constructing a hierarchy of temporal kernels grounded in empirical spectral scaling laws. On Human Connectome Project fMRI, PIMSM improves robustness and representation stability over parameter-matched baselines under severe temporal-context truncation, extreme low-resource transfer, and resting-state to task-state generalization. We further evaluate whether the same spectrum-guided temporal parameterization transfers to Weather-5K forecasting without domain-specific adaptation. These results suggest that generalizable sequence modeling of multiscale physical signals benefits from architectural alignment with the intrinsic temporal organization of the underlying dynamical system.
\end{abstract}
...
We evaluate PIMSM across temporal-context truncation, low-resource transfer, brain-state shift, and cross-domain forecasting (Weather-5K: calendar, temporal-shift, and spatial-shift splits), showing on completed HCP experiments that physics-informed multi-scale temporal structure improves robustness and representation stability over parameter-matched single-scale baselines without shift-specific adaptation.
\end{itemize}
...
We evaluate across four axes: \textbf{Axis (i)} temporal-context shift, implemented as early-window truncation of task trials while retaining full-sequence spectral calibration; \textbf{Axis (ii)} low-resource transfer; \textbf{Axis (iii)} brain-state shift (resting $\leftrightarrow$ task); and \textbf{Axis (iv)} cross-domain shift (Weather-5K calendar, temporal-shift, and spatial-shift splits).
...
\textbf{Weather-5K}~\citep{han2024far}: 5,000 global weather stations; planned cross-domain benchmark under three splits---\textbf{calendar}, \textbf{temporal shift}, and \textbf{spatial shift} (held-out stations)---to test whether spectral multiscale structure generalizes to independently-arising physical dynamics without domain-specific adaptation.
...
Table~\ref{tab:weather} reserves entries for variable-wise MAE on temperature, sea-level pressure, dew point, wind rate, and wind angle components; the benchmark is evaluated under calendar, temporal-shift, and spatial-shift splits.
...
Variable-wise MAE is reported for temperature (TMP), sea-level pressure (SLP), dew point (DEW), wind rate, and wind angle components (cos/sin). PIMSM is applied without domain-specific adaptation; entries are pending completion of the Weather-5K experiment.
}
\label{tab:weather}
\end{table}
...
We introduced \textbf{Physics-Informed Multi-Scale Mamba (PIMSM)}, motivated by the hypothesis that \emph{temporal kernel mismatch is a central source of generalization failure in sequence models for multiscale physical signals}. By coupling discretization parameters to spectrum-derived knee frequencies and jointly anchoring both $\Delta$ and $A$ in acquisition time units, PIMSM constructs a structured multi-scale kernel family that reduces mismatch under temporal-context, brain-state, and cross-domain shifts---without shift-specific adaptation.

On completed HCP experiments, PIMSM improves robustness over parameter-matched single-scale baselines under severe temporal-context truncation, extreme low-resource transfer, and resting-state$\to$task-state generalization. Ongoing Weather-5K experiments test whether the same spectrum-guided temporal parameterization extends to meteorological forecasting under calendar, temporal-shift, and spatial-shift splits. Together, these results support the view that robust sequence modeling of multiscale physical signals benefits from architectural alignment with the intrinsic temporal organization of the underlying dynamical system.
