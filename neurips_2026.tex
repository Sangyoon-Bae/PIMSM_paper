...
We evaluate PIMSM across temporal-context truncation, low-resource transfer, brain-state shift, and cross-domain forecasting (Weather-5K spatial OOD with held-out stations), showing that physics-informed multi-scale temporal structure improves robustness across both neural and meteorological time series without shift-specific adaptation.
\end{itemize}
...
\textbf{Weather-5K}~\citep{han2024far}: 5,000 global weather stations; cross-domain benchmark using hourly multivariate forecasting under a spatial OOD split, where geographically isolated held-out stations are never observed during training. This setting is deliberately modality-agnostic: the same temporal-scale parameterization used for fMRI is applied to meteorological variables sampled every hour.
...
We evaluate PIMSM across four axes of structured distribution shift. Axes (i--ii) use HCP motor fMRI; axis (iii) uses HCP resting-state$\to$task fMRI; axis (iv) uses Weather-5K spatial OOD forecasting. All comparisons use parameter-matched baselines (Mamba2, Transformer, 1D-CNN; $\approx$7.85M parameters each).
...
Variable-wise spatial OOD MAE is reported for temperature (TMP), sea-level pressure (SLP), dew point (DEW), wind rate, and wind angle components (cos/sin) across 24h, 72h, 120h, and 168h forecasting horizons. PIMSM is applied without domain-specific adaptation.
}
\label{tab:weather}
\end{table}
...
The current experiments test temporal-context truncation rather than a scanner-level change in TR; SpectralHyperNet receives the full sequence for scale calibration while the SSM backbone receives the shortened early-window input. This setting is useful for testing whether full-sequence spectral structure stabilizes representations under limited task evidence, but it should not be interpreted as a complete evaluation of acquisition-rate shift. PIMSM also assumes that a small number of spectrum-derived knees provides a useful summary of the signal's temporal organization; domains without clear piecewise spectral structure may require different scale parameterizations. The present model fixes $K=3$ scale regimes in the main experiments, so broader sensitivity over the number of knees remains future work. The acquisition-unit anchoring also requires a known sampling interval or physically meaningful time unit. SpectralHyperNet adds FFT and spectral-fit overhead relative to plain Mamba2, and the current manuscript does not provide a full GPU-hour accounting. Finally, the HCP tables use three seeds and report standard deviations rather than formal paired significance tests; the related multi-scale SSM/Mamba methods are discussed as contemporaneous related work but are not included as parameter-matched HCP baselines in the current tables. The Weather-5K evaluation focuses on spatial OOD generalization across held-out stations.
