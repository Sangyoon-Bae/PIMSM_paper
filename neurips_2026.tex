...
Given a multivariate time series $x_{1:T}$, a temporal encoder $\Phi_{\theta}$ produces latent states $h_{1:T}=\Phi_{\theta}(x_{1:T})$.
An aggregation map $\rho$ produces a representation $z=\rho(h_{1:T})$, and a task head $g_{\psi}$ predicts $\hat y=g_{\psi}(z)$.
Training on source condition $e$ minimizes empirical risk,
\begin{equation}
\min_{\theta,\psi}\;
\mathbb{E}_{(x,y)\sim \mathcal{D}_{e}}
\left[
\ell\!\left(g_{\psi}(\rho(\Phi_{\theta}(x))),y\right)
\right].
\label{eq:erm_main}
\end{equation}
The central question is whether the learned encoder remains useful under $e'$ without shift-specific retraining or adaptation.
...
\textbf{(b1--b3)} The knees partition the frequency axis, not the time series; an energy-weighted representative frequency from each regime is mapped to ordered discretization parameters ($\Delta_1 \ge \Delta_2 \ge \Delta_3$) for fast-to-slow SSM heads, whose states interact through cross-scale attention.
}
\label{fig:architecture_multiscale}
\end{figure}

PIMSM integrates multi-scale temporal structure into a state-space framework without relying on modality-specific components. As shown in Fig.~\ref{fig:architecture_multiscale}, the model first computes the power spectral density (PSD) of the input sequence and uses SpectralHyperNet to estimate knee frequencies $(f_1,f_2)$ and scaling exponents $(\beta_1,\beta_2,\beta_3)$. These parameters define a differentiable piecewise power-law approximation to the observed PSD, and the spectral fit loss compares the reconstructed piecewise PSD against the original PSD. The knees then partition the frequency axis into three regimes, corresponding to slow, intermediate, and fast temporal structure.

For each spectral regime, PIMSM computes an energy-weighted representative frequency $f_{c,k}$ under the fitted power-law slope rather than using a fixed band midpoint. This representative frequency is mapped through a monotonic frequency-to-discretization transformation to produce a scale-specific SSM timestep $\Delta_k$, with higher representative frequencies assigned larger $\Delta$ and lower representative frequencies assigned smaller $\Delta$. Importantly, this procedure does not split or filter the input time series; the knee and exponent estimates only determine the scale-specific $\Delta_k$ values. Groups of SSM heads share the same $\Delta_k$, so each group implements a distinct lag-weighting profile while processing the original time-domain sequence, reducing temporal-kernel mismatch under structured shifts. The resulting scale-specific latent states are passed through lightweight cross-scale attention to model interactions among temporal scales, allowing fast and slow temporal evidence to modulate one another before the pooled representation is passed to a task-specific head for classification or forecasting.
...
\midrule
\textbf{Input:} time-domain sequence $x_{1:T}$ for the SSM backbone; spectral context $x^{\mathrm{spec}}$ (the same sequence by default, or a longer full-trial/full-run context in early-window HCP experiments); frequency bounds $[f_{\min}, f_{\max}]$; number of scales $K=3$. \\
\textbf{1. PSD estimation:} compute $P(f)=|\mathrm{FFT}(x^{\mathrm{spec}})|^2$ over non-DC frequencies in $[f_{\min}, f_{\max}]$. \\
\textbf{2. Piecewise spectral fit:} SpectralHyperNet predicts knees $(f_1,f_2)$ and exponents $(\beta_1,\beta_2,\beta_3)$; a differentiable piecewise power-law $\hat P(f)\propto f^{-\beta_k}$ is fit to $P(f)$ with spectral-fit and seam-continuity losses. \\
\textbf{3. Representative frequencies:} the knees define frequency bands $[f_{\min},f_1]$, $[f_1,f_2]$, and $[f_2,f_{\max}]$; for each band, compute $f_{c,k}=\int fP_k(f)df/\int P_k(f)df$ under the fitted exponent $\beta_k$. \\
\textbf{4. Delta mapping:} map each $f_{c,k}$ to a scale-specific timestep $\Delta_k$ using $\Delta_k \propto f_{c,k}$; in the default implementation, $m_k=p_k(1-w)+w\,t_k$ with $w=0.3$ and $\Delta_k=\Delta_{\min}+(\Delta_{\max}-\Delta_{\min})m_k$, followed by the ordered constraint $\Delta_1\ge\Delta_2\ge\Delta_3$. \\
