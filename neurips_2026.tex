...
PIMSM supports temporal-scale alignment as a practical inductive bias for scientific sequence models. By coupling spectrum-derived knees to scale-specific $\Delta$ values and regularizing $A$ to prevent compensatory rescaling, the backbone learns a memory policy tied to the signal's physical organization rather than to a split-specific hyperparameter setting.

This matters when supervision is weak, observation is short, or deployment state changes. The HCP experiments cover early-window decoding, scarce labels, and resting-state$\rightarrow$task transfer; Weather-5K shows that the same spectrum-to-memory mechanism applies outside neuroimaging. The stronger gains at longer weather horizons are consistent with spectrum-derived scale assignment allocating memory across empirical slow and fast regimes, beyond simply using a long-memory Mamba backbone.

Limitations remain. The HCP temporal-context experiments test input truncation rather than scanner-level TR changes, because SpectralHyperNet receives the full trial for scale calibration while the SSM backbone receives shortened input. PIMSM assumes that a small number of spectrum-derived knees summarizes temporal organization; domains without clear piecewise spectra, or requiring a different scale count than the default $K=3$, may require different parameterizations. Acquisition-unit anchoring requires known sampling intervals, SpectralHyperNet adds FFT/spectral-fit overhead, HCP results use three seeds without formal significance claims, and contemporaneous multi-scale SSM/Mamba methods are discussed but not included as parameter-matched HCP baselines.
