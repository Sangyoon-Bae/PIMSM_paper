...
Weather-5K shows that the same mechanism is not tied to neuroimaging-specific assumptions. Under held-out-station spatial OOD, the larger gains at longer horizons suggest that robust forecasting increasingly depends on preserving longer-range temporal structure; this is consistent with spectrum-derived scale assignment allocating SSM memory across empirical slow and fast regimes, beyond simply giving Mamba a long-memory backbone. This supports the interpretation of PIMSM as a modality-agnostic spectrum-to-memory mechanism.
